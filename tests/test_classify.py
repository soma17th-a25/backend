...
from constants import CLARIFY_THRESHOLD
from utils.keyword_fallback import ClassificationOutput
...
async def test_classify_accuracy():
    ...
    for sc in scenarios:
        ...
        with patch("agents.classify.call_flash_json", side_effect=mock_fn):
...
async def test_classify_fallback_on_api_error():
    ...
    with patch("agents.classify.call_flash_json", side_effect=_raise):
