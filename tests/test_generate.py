...
from agents.generate import generate_node
...
async def _fake_stream(answer: str):
    """call_pro_stream의 AsyncIterator 모사."""
...
async def test_generate_includes_citations():
    ...
    with patch("agents.generate.call_pro_stream", side_effect=_stream_mock):
...
async def test_generate_no_hallucinated_law():
    """검색 결과에 없는 법조항을 답변에 포함하지 않아야 함 (간단 키워드 체크)."""
    ...
    # 검색 결과에 없는 법령 번호가 만들어지지 않았는지 확인 (허가된 doc 제목만 참조)
    allowed_titles = {"도로교통법 제44조", "2022도1234"}
    hallucinated = "도로교통법 제999조" not in result["answer_text"]
    assert hallucinated


@pytest.mark.asyncio
async def test_generate_no_settlement_section_when_none():
    """settlement이 None일 때 답변에 합의금 섹션이 없어야 함."""
...
async def test_generate_fallback_on_api_error():
    ...
    from llm.solar_client import SolarAPIError

    async def _raise(*args, **kwargs):
        raise SolarAPIError("테스트 오류")

    with patch("agents.generate.call_pro_stream", side_effect=_raise):
        result = await generate_node(_make_state())
