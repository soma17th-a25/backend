"""post_check_node — 룰 기반 신뢰도 평가, 변호사 권유 분기."""
from app.state import AgentState


def _score(state: AgentState) -> float:
...
async def post_check_node(state: AgentState) -> dict:
    score = _score(state)
    settlement = state.get("settlement")
    settlement_unreliable = bool(settlement) and not settlement.get("reliable", False)

    recommend = (
        score < 0.55
        or state.get("classification_confidence", 1.0) < 0.6
        or len(state.get("citations", [])) < 2
