"""Upstage Solar API 래퍼 — Flash(JSON) / Pro(스트리밍). openai SDK 사용."""
import logging
import os
import time
...
from constants import GEMINI_FLASH_MODEL, GEMINI_PRO_MODEL
...
async def call_flash_json(
    ...
    """Solar mini 호출, JSON 응답을 Pydantic으로 파싱 후 반환."""
    client = _get_client()
    start = time.perf_counter()
    logger.info("call_flash_json 시작 (model=%s)", GEMINI_FLASH_MODEL)
...
async def call_pro_stream(
    ...
    client = _get_client()
    start = time.perf_counter()
    logger.info("call_pro_stream 시작 (model=%s)", GEMINI_PRO_MODEL)
