"""classify_node — 사용자 질문을 case_type으로 분류."""
import json
import logging

from llm.solar_client import SolarAPIError, call_flash_json
from state import LegalState
from utils.keyword_fallback import ClassificationOutput, classify_by_keyword
...
def _build_user_prompt(state: LegalState, few_shot_block: str) -> str:
    ...
    recent = history[-4:] if len(history) > 4 else history  # 최근 2턴(user+assistant 각 2)
    history_text = ""
    if recent:
        history_text = "\n이전 대화:\n" + "\n".join(
...
async def classify_node(state: LegalState) -> dict:
    """
    State에서 user_query, history를 읽어 case_type 분류.
    ...
    # OUT_OF_SCOPE / RECKLESS_DRIVING는 needs_settlement 강제 false
