...
from constants import DISCLAIMER, MAX_CONTEXT_DOCS, MAX_HISTORY_TURNS
from llm.solar_client import SolarAPIError, call_pro_stream
from state import Citation, LegalState, RetrievedDoc
from taxonomy import CASE_TYPE_KOREAN
from utils.citation_extractor import extract_citations
from utils.prompt_loader import fill_template, load_prompt
...
async def generate_node(state: LegalState) -> dict:
    """
    State 전체를 컨텍스트로 묶어 Gemini Pro에 전달.
    토큰 스트리밍으로 답변 생성, 종료 후 인용 마커 추출.
    ...
    try:
        ...
        logger.warning("Gemini Pro 실패, 폴백 답변 사용: %s", exc)
...
async def generate_node_stream(state: LegalState) -> AsyncIterator[str]:
    ...
    try:
        ...
        logger.warning("Gemini Pro 스트리밍 실패: %s", exc)
